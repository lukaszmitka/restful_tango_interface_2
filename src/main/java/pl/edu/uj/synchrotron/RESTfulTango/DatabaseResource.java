...
/**
 * Main class of RESTfulTango.
 */
@Path("/{host}:{port}")
@Api(value = "Tango", description = "Access to Tango database")
public class DatabaseResource implements TangoConst {
	@GET
	@Produces(MediaType.TEXT_HTML)
	@ApiOperation(value = "List all devices", notes = "Returns list of all devices stored in database")
	public String devicesHtmlDefaulAction(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
	...
	@GET
	@Path("/Device/{trackStatus}")
	@Produces({ "application/json", "application/xml" })
	@ApiOperation(value = "List all devices", notes = "Returns list of all devices stored in database")
	public Response devicesJsonDefaultAction(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host,
			@ApiParam(value = "Port of Tango database", required = true) @PathParam("port") String port,
			@ApiParam(value = "Define if track device statuses", allowableValues = "true,false", required = true) @PathParam("trackStatus") boolean trackStatus) {
		String devices[];
		...
		try {
			...
			e.printStackTrace();
		} // System.out.println("Header contain " + header.length() +
			// " values");
	...
	@ApiOperation(
			...
			notes = "Returns list of all devices stored in database, sorted accordingly to their class, device server or domain.")
	public Response devicesJsonSortedDeviceList(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host,
			@ApiParam(value = "Port of Tango database", required = true) @PathParam("port") String port,
			@ApiParam(value = "Type of sorting", allowableValues = "1,2,3", required = true) @PathParam("sorting_type") String sortType,
			@ApiParam(value = "Define if track device statuses", required = true) @PathParam("trackStatus") boolean trackStatus) {
		JSONObject retJSONObject = new JSONObject();
		String devices[];
		try {
			try {
										...
										for (int l = 0; l < devList.length; l++) {
											...
											if (trackStatus) {
												try {
													...
													dp.ping();
													retJSONObject.put(devList[l].toUpperCase() + "isDeviceAlive" + l, true);
													// System.out.println("Device "+classes[j]+"is alive");
												} catch (DevFailed e) {
													retJSONObject.put(devList[l].toUpperCase() + "isDeviceAlive" + l, false);
												}
											}
										}
									}
	...
	@Path("/Device/{domain}/{class}/{member}/get_info")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Get device info", notes = "Returns information about selected device.")
	public Response deviceJsonGetInfo(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	@PUT
	@Path("/Device/{domain}/{class}/{member}/set_source/{source}")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Set source of device data", notes = "Returns information if task was executed properly.")
	public Response deviceJsonSetSource(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Source of device data, 0 - CACHE, 1 - CACHE_DEVICE, 2 - DEVICE", allowableValues = "0,1,2",
					required = true) @PathParam("source") String source) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	@GET
	@Path("/Device/{domain}/{class}/{member}/get_source")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Get source of device data", notes = "Returns the device data source .")
	public Response deviceJsonGetSource(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	@GET
	@Path("/Device/{domain}/{class}/{member}/get_device_info")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Get device info", notes = "Returns information about device.")
	public Response deviceJsonGetDeviceInfo(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	@GET
	@Path("/Device/{domain}/{class}/{member}/ping_device")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Pings device", notes = "Returns information if device responds.")
	public Response deviceJsonPingDevice(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	@GET
	@Path("/Device/{domain}/{class}/{member}/poll_status")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Poll device",
			notes = "Polls the device, returns values of attributes which are being polled.")
	public Response deviceJsonPollStatus(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
		...
		try {
			try {
				...
				try { // TODO Sprawdzić czy potrzebne jest get_adm_dev
	...
	@PUT
	@Path("/Device/{domain}/{class}/{member}/restart")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Restart device",
			notes = "Restarts the device, returns information if command was executed properly.")
	public Response deviceJsonRestart(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	@ApiOperation(value = "Get properties of device", notes = "Lists all device properties.")
	public String deviceHtmlGetPropertyList(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	@Path("/Device/{domain}/{class}/{member}/get_property_list")
	@ApiOperation(value = "Get properties of device", notes = "Lists all device properties.")
	public Response deviceJsonGetPropertyList(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	* public String deviceGetProperty(@PathParam("host") String host,
	* 
	* @PathParam("port") String port,
	* 
	* @PathParam("domain") String devDomain,
	* 
	* @PathParam("class") String devClass,
	* 
	* @PathParam("member") String devMember,
	* 
	* @PathParam("prop_name") String propName) { String retStr =
	* "<html><title>Device " + devDomain + "/" + devClass + "/" + devMember +
	* "</title>" + "<body><h1>Device : " + devDomain + "/" + devClass + "/" +
	* devMember + "</h1>"; try { DeviceProxy dp = new DeviceProxy(devDomain +
	* "/" + devClass + "/" + devMember, host, port); DbDatum devDbDatum =
	* dp.get_property(propName); String propValue = devDbDatum.extractString();
	* retStr = retStr + "Property name: " + propName + ", Value: " + propValue;
	* } catch (DevFailed e) { retStr = retStr +
	* "Unable to connect to device<br>"; e.printStackTrace(); } return retStr +
	* "</body></html>"; }
	*/
	/* @PUT
	* 
	...
	* 
	* @Path(
	* "/Device/{domain}/{class}/{member}/put_property/{prop_name}/{prop_value}"
	* ) public String deviceHtmlPutProperty(@PathParam("host") String host,
	* 
	* @PathParam("port") String port,
	* 
	* @PathParam("domain") String devDomain,
	...
	* @PathParam("member") String devMember,
	* 
	* @PathParam("prop_name") String propName,
	* 
	* @PathParam("prop_value") String propValue) { String retStr =
	* "<html><title>Device " + devDomain + "/" + devClass + "/" + devMember +
	* "</title>" + "<body><h1>Device : " + devDomain + "/" + devClass + "/" +
	* devMember + "</h1>"; try { DeviceProxy dp = new DeviceProxy(devDomain +
	* "/" + devClass + "/" + devMember, host, port); DbDatum devDbDatum = new
	* DbDatum(propName, propValue); dp.put_property(devDbDatum); retStr =
	* retStr + "Property name: " + propName + ", New value: " + propValue; }
	* catch (DevFailed e) { retStr = retStr +
	* "Unable to connect to device<br>"; e.printStackTrace(); } return retStr +
	* "</body></html>"; }
	*/
	@PUT
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/set_timeout_milis/{timeout}")
	@ApiOperation(value = "Set device timeout", notes = "Set device timeout.")
	public Response deviceJsonSetTimeoutMilis(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Device timeout in miliseconds", required = true) @PathParam("timeout") String timeout) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/black_box/{nbCmd}")
	@ApiOperation(value = "Read device black box",
			notes = "Reads selected number of commands that was recently executed on device.")
	public Response deviceJsonBlackBox(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Number of commands", required = true) @PathParam("nbCmd") String nbCmd) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	@ApiOperation(value = "Update property value", notes = "Updates selected property with new value.")
	public Response deviceJsonPutProperty(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Property name", required = true) @PathParam("prop_name") String propName, @ApiParam(
					value = "Property value", required = true) @PathParam("prop_value") String propValue) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	* @Produces(MediaType.TEXT_HTML)
	* 
	* @Path("/Device/{domain}/{class}/{member}/get_attribute_list") public
	* String deviceHtmlGetAttributeList(@PathParam("host") String host,
	* 
	* @PathParam("port") String port,
	* 
	* @PathParam("domain") String devDomain,
	...
	* @PathParam("class") String devClass,
	* 
	* @PathParam("member") String devMember) { String retStr =
	* "<html><title>Device " + devDomain + "/" + devClass + "/" + devMember +
	* "</title>" + "<body><h1>Device : " + devDomain + "/" + devClass + "/" +
	* devMember + "</h1>"; try { DeviceProxy dp = new DeviceProxy(devDomain +
	* "/" + devClass + "/" + devMember, host, port); retStr = retStr +
	* "<h2>Device attributes</h2>"; String att_list[] =
	* dp.get_attribute_list(); for (int i = 0; i < att_list.length; i++) {
	* retStr = retStr + att_list[i].toString() + "<br>"; } } catch (DevFailed
	* e) { retStr = retStr + "Unable to connect to device<br>";
	* e.printStackTrace(); } return retStr + "</body></html>"; }
	*/
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/get_attribute_list")
	@ApiOperation(value = "Get device attributes", notes = "Lists all device attributes.")
	public Response deviceJsonGetAttributeList(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	* public String deviceReadAttribute(@PathParam("host") String host,
	* 
	* @PathParam("port") String port,
	* 
	* @PathParam("domain") String devDomain,
	...
	* @PathParam("member") String devMember,
	* 
	* @PathParam("att_name") String attName) { String retStr =
	* "<html><title>Device " + devDomain + "/" + devClass + "/" + devMember +
	* "</title>" + "<body><h1>Device : " + devDomain + "/" + devClass + "/" +
	* devMember + "</h1>"; try { DeviceProxy dp = new DeviceProxy(devDomain +
	* "/" + devClass + "/" + devMember, host, port); retStr = retStr +
	* "<h2>Device attribute " + attName + "</h2>"; DeviceAttribute da =
	* dp.read_attribute(attName); AttributeInfo ai =
	* dp.get_attribute_info(attName); String data = extractDataValue(da, ai);
	...
	* || ai.data_type == Tango_DEV_BOOLEAN)) { retStr = retStr +
	* "Attribute is not plottable<br>"; } else if (ai.data_format.value() ==
	* AttrDataFormat._SPECTRUM || ai.data_format.value() ==
	* AttrDataFormat._IMAGE) { retStr = retStr + "Attribute is plottable<br>";
	* } else { retStr = retStr + "Attribute is not plottable<br>"; }
	* 
	...
	* (ai.writable.value() == AttrWriteType._WRITE)) { retStr = retStr +
	* "Attribute is writable<br>"; } } catch (DevFailed e) { retStr = retStr +
	* "Unable to connect to device<br>"; e.printStackTrace(); } return retStr +
	* "</body></html>"; }
	*/
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/read_attribute/{att_name}")
	@ApiOperation(value = "Read attribute", notes = "Reads selected attribute, and returns its value.")
	public Response deviceJsonReadAttribute(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Attribute name", required = true) @PathParam("att_name") String attName) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	@ApiOperation(value = "Reads attributes", notes = "Reads values of attributes sent in JSON object.")
	public Response deviceJsonReadAttributes(
			@ApiParam(
					value = "List of attributes to be read. Should contain object \"attCount\" with number of attibutes to read and appropriate number of objects \"att\"+i, where \"i\" is number of attribute",
					required = true) String request,
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	public Response deviceJsonPlotAttribute(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("att_name") String attName) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
				...
				switch (ai.data_format.value()) {
					...
					case AttrDataFormat._IMAGE:
						...
						break;
					default:
						retJSONObject.put("dataFormat", "Attribute can not be plotted");
						break;
				}
	...
	* 
	* @Path(
	* "/Device/{domain}/{class}/{member}/write_attribute/{att_name}/{att_value}"
	* ) public String deviceHtmlWriteAttribute(@PathParam("host") String host,
	* 
	* @PathParam("port") String port,
	* 
	* @PathParam("domain") String devDomain,
	...
	* @PathParam("member") String devMember,
	* 
	* @PathParam("att_name") String attName,
	* 
	* @PathParam("att_value") String attValue) { String retStr =
	* "<html><title>Device " + devDomain + "/" + devClass + "/" + devMember +
	* "</title>" + "<body><h1>Device : " + devDomain + "/" + devClass + "/" +
	* devMember + "</h1>"; try { DeviceProxy dp = new DeviceProxy(devDomain +
	* "/" + devClass + "/" + devMember, host, port); retStr = retStr +
	* "<h2>Device attribute " + attName + "</h2>"; DeviceAttribute da =
	* dp.read_attribute(attName); AttributeInfo ai =
	* dp.get_attribute_info(attName); da = insertData(attValue, da, ai);
	* dp.write_attribute(da); String data = extractDataValue(da, ai); retStr =
	* retStr + "Attribute: " + attName + ", new value: " + data; } catch
	* (DevFailed e) { retStr = retStr + "Unable to connect to device<br>";
	* e.printStackTrace(); } return retStr + "</body></html>"; }
	*/
	@PUT
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/write_attribute/{att_name}/{att_value}")
	@ApiOperation(value = "Write attribute", notes = "Writes selected attribute with new value.")
	public Response deviceJsonWriteAttribute(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Attribute name", required = true) @PathParam("att_name") String attName, @ApiParam(
					value = "Attribute value", required = true) @PathParam("att_value") String attValue) {
		JSONObject retJSONObject = new JSONObject();
		try {
			try {
				System.out.println("Connecting to device");
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/command_list_query")
	@ApiOperation(value = "Get command list", notes = "Gets list of all device commands")
	public Response deviceJsonCommandListQuery(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	...
	@PUT
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/command_inout/{commandName}/{argin}")
	@ApiOperation(
			value = "Execute command",
			notes = "Executes selected command with specified argument. If command has no argument (void), as argument should be passed \"DevVoidArgument\"")
	public Response deviceJsonCommandInOut(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
					value = "Device instance", required = true) @PathParam("member") String devMember, @ApiParam(
					value = "Command name", required = true) @PathParam("commandName") String commandName, @ApiParam(
					value = "Argument", required = true) @PathParam("argin") String inputArgument) {
		JSONObject retJSONObject = new JSONObject();
		...
		try {
			try {
				System.out.println("Connecting to device: " + devDomain + "/" + devClass + "/" + devMember);
				DeviceProxy dp = new DeviceProxy(devDomain + "/" + devClass + "/" + devMember, host, port); // CommandInfo
	...
	@PUT
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Device/{domain}/{class}/{member}/extract_plot_data/{commandName}/{argin}")
	@ApiOperation(
			value = "Plot command",
			notes = "Executes selected command with specified argument. If command has no argument (void), as argument should be passed \"DevVoidArgument\". Return command output as array.")
	public Response deviceJsonCommandPlot(
			@ApiParam(value = "Address of Tango database", required = true) @PathParam("host") String host, @ApiParam(
					value = "Port of Tango database", required = true) @PathParam("port") String port, @ApiParam(
					value = "Device domain", required = true) @PathParam("domain") String devDomain, @ApiParam(
					value = "Device class", required = true) @PathParam("class") String devClass, @ApiParam(
	 ...
	 */
	/*
	 * private boolean isPlotable(AttributeInfo ai) { if ((ai.data_type ==
	 * Tango_DEV_STRING) || (ai.data_type == Tango_DEV_STATE) || (ai.data_type
	 * == Tango_DEV_BOOLEAN)) return false; return (ai.data_format.value() ==
	 * AttrDataFormat._SPECTRUM) || (ai.data_format.value() ==
	 ...
	 */
	/*
	 * private boolean isWritable(AttributeInfo ai) { return
	 * (ai.writable.value() == AttrWriteType._READ_WITH_WRITE) ||
	 * (ai.writable.value() == AttrWriteType._READ_WRITE) ||
	 ...
	 */
	/*
	 * private AttributeInfo[] getAttributeList() throws DevFailed { int i, j;
	 * boolean end; AttributeInfo tmp; AttributeInfo[] lst =
	 * device.get_attribute_info(); // Sort the list end = false; j = lst.length
	 * - 1; while (!end) { end = true; for (i = 0; i < j; i++) { if
	 * (lst[i].name.compareToIgnoreCase(lst[i + 1].name) > 0) { end = false; tmp
	 * = lst[i]; lst[i] = lst[i + 1]; lst[i + 1] = tmp; } } j--; } return lst; }
	 */
	/**
